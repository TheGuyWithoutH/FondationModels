...
class MaskGIT(nn.Module):
    ...
    def __init__(
        ...
        self.mask_token = nn.Parameter(torch.randn(dim)) # Mask token for masked positions
    ...
    def forward_model(self, x: torch.LongTensor, mask: torch.BoolTensor) -> torch.Tensor:
        ...
        mask_tokens = self.mask_token.unsqueeze(0).unsqueeze(0)  # [1, 1, D]
        mask = mask.unsqueeze(-1)  # [B, L, 1]
        embeddings = torch.where(mask, mask_tokens.expand_as(embeddings), embeddings)
    ...
    def generate_random_mask(self, seq: torch.Tensor) -> torch.BoolTensor:
        ...
        # Generate random mask lengths for each sample in the batch (between 1 and L)
        mask_lengths = torch.randint(1, L + 1, (B,), device=seq.device)
        
        # Create mask indices tensor: for each sample, generate random indices to mask
        mask = torch.zeros((B, L), dtype=torch.bool, device=seq.device)
        
        # For each sample in the batch, generate a random permutation of indices
        # and select the first mask_lengths[i] indices for masking
        for i in range(B):
            # Generate random permutation of indices for this sample
            perm = torch.randperm(L, device=seq.device)
            # Select the first mask_lengths[i] indices to mask
            indices_to_mask = perm[:mask_lengths[i]]
            # Set these indices to True in the mask for this sample
    ...
    def generate(
        ...
        for step, k in enumerate(schedule):
            ...
            M = masked_indices.size(0)
            assert M > 0, "No masked tokens to unmask in the input sequence."
            ...
            top_k_indices = torch.topk(confidence, schedule[step], dim=0).indices
